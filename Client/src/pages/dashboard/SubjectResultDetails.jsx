  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4">
        <Link to="/dashboard/results" className="p-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-gray-400 hover:text-white transition-colors shrink-0 self-start md:self-auto">
          <ArrowLeft size={24} />
        </Link>
        <div>
          <h1 className="text-3xl font-black text-white">{subject ? subject.subject : "Subject Results"}</h1>
          <p className="text-gray-400 mt-1">{subject ? `${subject.examName} | ${subject.session}` : "Class performance breakdown"}</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900/50">
                ...
                <th className="p-4 font-semibold text-gray-400 text-sm">AI Score (if any)</th>
                ...
                return (
                  <tr key={ev._id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 text-gray-400 text-sm">
                      {ev.aiEvaluationId ? "Checked" : "-"}
                    </td>
                    <td className="p-4">
                      <Link 
                        to={`/dashboard/digital-evaluation/${ev.answerSheetId}`}
